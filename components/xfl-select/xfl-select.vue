<template>
  <div class="show-box" :class="{disabled: disabled, active: isShowList}" :style="style_Container">
    <!-- 输入框，仅在可输入模式下使用 -->
    <span class="showtxt">{{showString}}</span>
    ...
    <!-- 显示框 -->
    <div v-else class="input" :class="{placeholder: selectText === placeholder}" @click="onUpperClick">{{selectText}}
    </div>
    ...
    <!-- 列表框 -->
    <div class="list-container" @click.stop="onListClick" style="top:84rpx;" v-show="isShowList">
  ...
  methods: {
    ...
    onClickItem (index, value) {  // 列表项上的点击事件
      ...
      if (!this.itemIsActive(index)) {  //如果点在非选中项上
        ...
        this.$emit('change', {
          newVal: value, oldVal: this.selectText,
...
.show-box {
  ...
  position: relative;
  background: #ffffff;
  box-shadow: 0 0 18rpx 0 rgba(96, 109, 184, 0.17);
  border-radius: 20rpx;
  .input {

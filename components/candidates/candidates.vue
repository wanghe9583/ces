<template>
	<view>
	<text @click="open=true" v-show="!open">{{title?title:''}}</text>
	<view class="uni-picker-container" v-if="open">
		<view class="uni-mask">
		<view class="choose">
...
<script>
	export default {
		...
		methods: {
			confirm(){
				// let e=this.chooseValueList;
				// e.forEach(x=>{
				// 	delete x.isSelected;
				// })
				// if(this.chooseValueList.length){
				// 	this.chooseValueList.forEach(x=>{
				// 		this.departmentList.forEach((h,index)=>{
				// 			this.PersonnelList[index].forEach(e=>{
				// 				if(x.rid==e.rid){
				// 					e.isSelected=true;
				// 				}
				// 			})
				...
				// 	})
				// }
				this.dispose(this.chooseValueList)
				this.open=false;
			},
			show() {
			   this.open = true;
